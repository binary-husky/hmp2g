...
class 三体运动仿真():
    def __init__(self) -> None: # 初始条件 initial position
        ...
        self.位置 = 8*np.array([[0,0,1]]+  [  [cos(d_theta*i),    sin(d_theta*i),  0] for i in range(self.n体)  ])  # the position of entities
        self.速度 = 2*np.array([[0,0,0]]+  [  [cos(d_theta*i+PI/2),    sin(d_theta*i+PI/2),  0] for i in range(self.n体) ])  # the vel of entities
        self.质量 = np.array( [25]+[     1   for i in range(self.n体)  ] )  # mass
        self.n体+=1
