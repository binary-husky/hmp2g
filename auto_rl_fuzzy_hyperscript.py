...
"""
{
    "config.py->GlobalConfig": {
        ...
        "backup_files": [
            "ALGORITHM/experimental_conc_mt_fuzzy_agent_wise_2",
    ...
    "MISSION.dca_multiteam.collective_assult_parallel_run.py->ScenarioConfig": {
        ...
        "TEAM_NAMES": [
            "ALGORITHM.experimental_conc_mt_fuzzy_agent_wise_2.foundation->ReinforceAlgorithmFoundation",
            // "ALGORITHM.random.foundation->RandomController"
            "TEMP.TEAM2.ALGORITHM.experimental_conc_mt_fuzzy_agent_wise_2.foundation->ReinforceAlgorithmFoundation"
        ]
    },
    "ALGORITHM.experimental_conc_mt_fuzzy_agent_wise_2.foundation.py->AlgorithmConfig": {
        ...
        "fuzzy_controller_param": [2, 1, 0, 3, 1 ],
    ...
    "TEMP.TEAM2.ALGORITHM.experimental_conc_mt_fuzzy_agent_wise_2.foundation.py->AlgorithmConfig": {
        "train_traj_needed": 32,
        "n_focus_on": 4,
        "lr": 0.0003,
        "ppo_epoch": 16,
...
MasterAutoRLKey = 'auto_rl_fuzzy_agentwise_entropy'
...
class HmpBayesianOptimizationInterface(BayesianOptimizationInterface):
    ...
    def post_part(self, X, get_score, future_list, batch):
        ...
        self.logger.debug(
                f'input X={X} | output {y_array}, average {y}')
        res = (y - self.y_offset)
    ...
    def prev_part(self, X, batch):
        ...
        p6 = self.convert_continuous( from_x = X[5], to_range=[0,          1], p_index=5)
    ...
    def get_device_conf(self, batch):
        if batch==0:
            ...
            return {
                ...
                "ALGORITHM.experimental_conc_mt_fuzzy_agent_wise_2.foundation.py->AlgorithmConfig-->gpu_party_override":
                    [
                        "cuda0_party1", # 各子实验的party可以相同， 但每个实验的子队伍party建议设置为不同值
                        "cuda0_party1",
                        "cuda1_party1", # 各子实验的party可以相同， 但每个实验的子队伍party建议设置为不同值
                        "cuda1_party1",
                    ],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=3, help='batch size for BO.')
