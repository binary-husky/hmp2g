...
color = ['red','green','blue','DeepPink','yellow', 'Cyan','Chocolate','red','green','blue','DeepPink',]
class 三体运动仿真():
    def __init__(self) -> None: # 初始条件 initial position
        self.n体 = 3
        d_theta = np.pi*2/self.n体
        self.位置 = 30*np.array([[0,0,1/5], [0,0,-1/5]]+  [  [cos(d_theta*i),    sin(d_theta*i),  0] for i in range(self.n体)  ])  # the position of entities
        self.位置 += 2*(np.random.random(size=self.位置.shape) - 0.5)
        self.速度 = 5*np.array([[0,0,1],[0,0,-1]]+  [  [cos(d_theta*i+PI/2),    sin(d_theta*i+PI/2),  0] for i in range(self.n体) ])  # the vel of entities
    ...
    def sim_and_render(self):
        self.physic_sim()
        if self.step%100==0: self.render()
    ...
    def render(self):
        ...
        for index in range(self.n体):
            ...
            self.可视化桥.line3d(
                'fat|%d|%s|%.3f'%(index+200, color[index], 0.04),
