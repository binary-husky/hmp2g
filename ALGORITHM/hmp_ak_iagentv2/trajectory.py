...
class trajectory(TRAJ_BASE):
    ...
    def __init__(self, traj_limit, env_id, mcv):
    ...
    def cut_tail(self): # get_hyper_reward(self):
        # 删去多余的预留空间
        super().cut_tail()
    ...
    def finalize(self):
        ...
        # deadmask
        dead_mask = (my_view(self.obs, [0,0,-1])==0).all(-1)
        ...
        if np.percentile(s_time, 50) < 100:
            ...
            # filter = filter & (s_time != self.time_pointer)
...
class BatchTrajManager():
    ...
    def __init__(self, n_env, traj_limit, trainer_hook, mcv):
        ...
        self.mcv = mcv
        self.live_trajs = [trajectory(self.traj_limit, env_id=i, mcv=mcv) for i in range(n_env)]
    ...
    def __batch_update(self, traj_frag):
        ...
        for env_i in range(self.n_env):
            ...
            for key in traj_frag:
                self.make_traj_remember(traj=traj_handle, key=key, content=traj_frag[key],frag_index=frag_index, n_active=n_active)
            ...
            if done[env_i]:
                ...
                self.live_trajs[env_index] = trajectory(self.traj_limit, env_id=env_index, mcv=self.mcv)
                self.live_traj_frame[env_index] = 0
        # print红(self.live_traj_frame)
    def make_traj_remember(self, traj, key, content, frag_index, n_active):
        if content is None:
            traj.remember(key, None)
        elif isinstance(content, dict):
            for sub_key in content: 
                self.make_traj_remember(traj, "".join((key , ">" , sub_key)), content=content[sub_key], frag_index=frag_index, n_active=n_active)
        else:
