...
class Net(Logit2Act, nn.Module):
    def __init__(self,
        ...
        self.alg = alg
        self.use_normalization = self.alg.use_normalization
        self.n_focus_on = self.alg.n_focus_on
        self.adopt_selfattn = False
        self.dual_conc = self.alg.dual_conc
        self.stage_planner = stage_planner
        self.use_policy_resonance = self.alg.use_policy_resonance
        self.alternative_critic = self.alg.alternative_critic
        ...
        if self.dual_conc:
            self.MIX_conc_core_f = Concentration(
                            ...
                            adopt_selfattn=self.adopt_selfattn)
    ...
    def logit2act(self, logits_agent_cluster, eval_mode, test_mode, eval_actions=None, avail_act=None):
        ...
        if eval_mode:
            act = eval_actions; assert (not test_mode) 
        elif test_mode:
            act = torch.argmax(act_dist.probs, axis=2)
        else:
            act = act_dist.sample()
        assert act.dim() == 2    # shape = (time/batch, agent)
    ...
    def _act(self, obs, test_mode, eval_mode=False, eval_actions=None, avail_act=None, eprsn=None, randl=None):
        eval_act = eval_actions if eval_mode else None
        others = {}
        if self.use_normalization:
        ...
        logit2act = self._logit2act_rsn_entropy_split \
                if self.alg.fuzzy_controller else self._logit2act_entropy_split
