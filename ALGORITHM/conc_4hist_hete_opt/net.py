...
from ALGORITHM.common.conc import Concentration, ConcentrationHete
...
class Net(nn.Module):
    def __init__(self, rawob_dim, n_action, **kwargs):
        ...
        self.AT_get_value = nn.Sequential(nn.Linear(tmp_dim, h_dim), nn.ReLU(inplace=True),nn.Linear(h_dim, 1))
        self.AT_get_threat = nn.Sequential(nn.Linear(tmp_dim, h_dim), nn.ReLU(inplace=True),nn.Linear(h_dim, 1))
    ...
    def _act(self, obs=None, test_mode=None, eval_mode=False, eval_actions=None, avail_act=None, agent_ids=None, eprsn=None):
        ...
        if eval_mode: 
            threat = self.AT_get_threat(v_M_fuse)
            value  = self.AT_get_value(v_M_fuse)
