...
class SimpleAttention(nn.Module):
    def __init__(self, in_dim, out_dim):
        super().__init__()
        self.W_query = nn.Parameter(torch.Tensor(in_dim, out_dim))
        self.W_key = nn.Parameter(torch.Tensor(in_dim, out_dim))
...
def weights_init(m):
    ...
    if classname ==  'Conv':
        ...
        return
    elif classname == 'SimpleAttention':
        return
...
class CNet(nn.Module):
    ...
    def __init__(self, num_agents, num_entities, basic_vec_len, hidden_dim=32):
        ...
        if self.sub_cluster_size != 1:
            # k = input, v = input, q = ones
            self.sub_cluster_processing_nn = SimpleAttention(agent_emb_dim, agent_emb_dim)
    ...
    def get_feature(self, all_emb, eval_mode):
        ...
        if self.sub_cluster_size != 1:
            agent_final_emb = my_view(agent_final_emb, [0,0, self.sub_cluster_size, -1])
            agent_final_emb = self._batch_norm_agent(agent_final_emb)
            agent_final_emb = self.sub_cluster_processing_nn(k=agent_final_emb, v=agent_final_emb, 
                q=torch.ones_like(agent_final_emb[..., 0, :]).unsqueeze(-2)).squeeze(-2)
        else:
            agent_final_emb = self._batch_norm_agent(agent_final_emb)
