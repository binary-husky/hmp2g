...
def make_env(env_id, seed, rank, num_agents, dist_threshold, arena_size, idtarget_size):
    def _thunk():
        env = make_multiagent_env(env_id, num_agents, dist_threshold, arena_size, idtarget_size, rank)
        return env
    return _thunk


def make_multiagent_env(env_id, num_agents, dist_threshold, arena_size, idtarget_size, rank):
    scenario = scenarios.load(env_id+".py").Scenario(num_agents=num_agents, dist_threshold=dist_threshold,
                                                     arena_size=arena_size, idtarget_size=idtarget_size, process_id=rank)
