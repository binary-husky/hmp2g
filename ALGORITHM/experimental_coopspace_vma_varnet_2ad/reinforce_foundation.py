...
def get_num_entity(ScenarioConfig):
    if hasattr(ScenarioConfig, "num_entity"):
        return ScenarioConfig.num_entity
    else:
        return 1
    
def get_agent_uids(ScenarioConfig, team):
    return ScenarioConfig.AGENT_ID_EACH_TEAM[team]
...
class ReinforceAlgorithmFoundation():
    def __init__(self, n_agent, n_thread, space, mcv=None, team=None):
        ...
        self.n_primitive_act = 6
        self.n_entity = self.n_primitive_act + get_num_entity(ScenarioConfig) # 6 raw action + num_entity coop actions
        self.ObsAsUnity = False
        if hasattr(ScenarioConfig, 'ObsAsUnity'):
            self.ObsAsUnity = ScenarioConfig.ObsAsUnity
        self.agent_uid = get_agent_uids(ScenarioConfig, team) # ScenarioConfig.uid_dictionary['agent_uid']
        self.target_uid = get_agent_uids(ScenarioConfig, 1-team)
        ...
        if self.load_checkpoint:
            ...
            self.policy.load_state_dict(torch.load(ckpt_dir, map_location=cuda_n), strict=False)
        #################### ?? ####################
    ...
    def action_making(self, State_Recall):
        ...
        if '_graphstate_' in State_Recall:
            ...
            self.coopgraph = CoopGraph(
                ...
                n_primitive_act = self.n_primitive_act,
                )
