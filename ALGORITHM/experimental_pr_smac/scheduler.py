...
class BottleneckPolicyResonance:
    def __init__(self, mcv) -> None:
        self.total_epoch = 0
        self.best = -np.inf
        self.num_bad_epochs = 0
        self.min_patience = 50
        self.cooldown = 50
        self.threshold = 0.01
        self.on = False
        self.mcv = mcv
    ...
    def step(self, metrics):
        ...
        if metrics > 0.1:
            ...
            patience = max(self.min_patience, self.total_epoch / self.n_th)
            # 
            if self.num_bad_epochs > patience:
                self._do_some_adjustment()
                self.num_bad_epochs = 0

        self.mcv.rec(self.num_bad_epochs, 'num_bad_epochs')
        self.mcv.rec(patience, 'patience')


    def _do_some_adjustment(self):
        self.on = True
