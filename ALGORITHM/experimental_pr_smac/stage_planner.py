...
class StagePlanner:
    def __init__(self, n_agent, mcv) -> None:
        ...
        elif PolicyRsnConfig.yita_shift_method == 'wait_bottleneck':
            from .scheduler import BottleneckPolicyResonance
            self.feedback_controller = BottleneckPolicyResonance(mcv)
        else:
    ...
    def _update_yita(self):
        ...
        if PolicyRsnConfig.yita_shift_method == '-cos':
            ...
            if self.feedback_controller.on: 
                self.yita = PolicyRsnConfig.yita_max
            else: 
                self.yita = 0
            print亮绿('yita update:', self.yita)
