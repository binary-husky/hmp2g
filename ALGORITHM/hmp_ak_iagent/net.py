...
class Pnet(nn.Module):
    ...
    def get_feature(self, obs, eval_mode):
        ...
        agent_enc  = self.AT_obs_encoder(obs) # Linear -> ReLu -> Linear, # torch.Size([64, 50, 192.coresize])
        if self.actor_attn_mod:
            agent_enc = self.AT_attention(agent_enc) # self-attn -> skip connect -> mlp (Linear -> ReLu -> Linear)
