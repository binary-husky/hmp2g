...
class trajectory(TRAJ_BASE):
    ...
    def finalize(self, winrate):
        ...
        if AlgorithmConfig.fuzzy_controller:
            # assert "entropy_controller" in GlobalConfig
            sigma = 10
            ...
            er = np.array([GlobalConfig.entropy_controller.compute_fn((l, wr)) for l in normalized_score])
            self.copy_track('reward', 'entropy_reg')
            n_agent = len(er)
            for i in range(n_agent): 
                self.entropy_reg[:, i] += er[i]
