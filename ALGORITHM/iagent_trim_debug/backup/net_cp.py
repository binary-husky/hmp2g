...
class Net(nn.Module):
    def __init__(self, num_agents, n_basic_dim, obs_dim, n_action, K_EMod=2, hidden_dim=128, use_m_gpu=None, use_normalization=True, seperate_critic=False):
        ...
        self.CT_encode = nn.Sequential(
                            nn.Linear(15*12, 64), lkrelu(),
                            nn.Linear(64, basic_vec_len))
        self.CT_attention = MultiHeadAttention(n_heads=1, input_dim=16, embed_dim=16)
        self.CT_getvalue = nn.Sequential(
            Linear(basic_vec_len, 16), lkrelu(inplace=True),
            Linear(16, 1),
        )
        # part3
        self.AT_obs_encoder = SimpleMLP(in_dim=12*8, out_dim=h_dim, use_normalization=False)
    ...
    def _act(self, obs, test_mode, eval_mode=False, eval_actions=None):
        ...
        # ! <<< Critic network >>>
        ct_phi = self.CT_encode( my_view(obs, [0, 0, -1]) )    # maybe here
        ...
        value = self.CT_getvalue(ct_phi)#torch.cat((ct_phi_self.squeeze(-2), ct_enc.squeeze(-2)), -1))
