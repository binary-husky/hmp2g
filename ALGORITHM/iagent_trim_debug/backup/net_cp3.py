...
class Net(nn.Module):
    def __init__(self, num_agents, n_basic_dim, obs_dim, n_action, K_EMod=2, hidden_dim=128, use_m_gpu=None, use_normalization=True, seperate_critic=False):
        ...
        # part1
        self.AE_encode = nn.Sequential(
                            nn.Linear(n_basic_dim, 32), lkrelu(inplace=True),
                            nn.Linear(32, 8), 
                        )

        self.AE_decode = nn.Sequential(
                            nn.Linear(8, basic_vec_len), lkrelu(inplace=True),
                            nn.Linear(basic_vec_len, basic_vec_len)
                        )

        # part2
        # self.CT_encode = ResLinear(io_dim=16, h_dim=64, need_input_tf=True, input_tf_dim=15, inplace_relu=False)
        ...
        # part3
        # self.AT_obs_encoder = SimpleMLP(in_dim=15*12, out_dim=h_dim, use_normalization=False)
        # self.AT_extract_mod = EXTRACT(self.K_EMod, h_dim)
        # self.AT_encode_00 = 
        # self.AT_encode = nn.Linear(45, 64)
        self.AT_get_logit = nn.Sequential(  
            nn.Linear(15*3, 64), nn.ReLU(inplace=True),
            nn.Linear(64, 64), nn.ReLU(inplace=True),
            nn.Linear(64, 64), nn.ReLU(inplace=True),
            LinearFinal(64, self.n_action))
    ...
    def _act(self, obs, test_mode, eval_mode=False, eval_actions=None):
        ...
        phi = self.AE_encode(obs_nan2zero) # phi (n_thread, n_agent, 12_agent+12agent ph, coredim=8)
        ...
        m_atten_softmax = m_atten_softmax.squeeze(-2)
        ...
        # phi2 = self.AT_encode_00(obs[:,:,(0,1,6)])
        # phi2_focus = self.AT_encode(my_view(phi2, [0, 0, -1]))
        ...
        logits = self.AT_get_logit(my_view(obs[:,:,(0,1,6)], [0, 0, -1]) )
