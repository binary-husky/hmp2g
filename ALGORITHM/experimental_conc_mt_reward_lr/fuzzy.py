...
def int2consq(a, fuzzy_consequent):
    ...
    return fuzzy_consequent[q[a]], q[a]
...
def gen_feedback_sys_generic_multi_input(
    ...
    if len(fuzzy_antecedents) >= 2:
        for combo in itertools.combinations(fuzzy_antecedents.items(), 2):
            antecedent_keyname_1, antecedent_1 = combo[0]
            antecedent_keyname_2, antecedent_2 = combo[1]
            for mf_1 in [ 'small', 'medium', 'large' ]:
                for mf_2 in [ 'small', 'medium', 'large' ]:
                    consquent_with_term, term = int2consq(fuzzy_controller_param[pointer], fuzzy_consequent)
                    rule_list.append(ctrl.Rule(
                        antecedent_1[mf_2] & antecedent_2[mf_1] ,  consquent_with_term
                    ))
                    print(f'{str(antecedent_1[mf_2])} & {str(antecedent_2[mf_1])} --> {str(consquent_with_term)}')
                    pointer += 1

            # rule_list.append(ctrl.Rule(
            #     fuzzy_antecedents['lifelen_norm']['small'] & fuzzy_antecedents['recent_winrate']['small'] ,  int2consq(fuzzy_controller_param[0], fuzzy_consequent)
            # ))
    elif len(fuzzy_antecedents) == 1:
        antecedent_keyname_1, antecedent_1 = list(fuzzy_antecedents.items())[0]
        for mf_3 in [ 'small', 'medium', 'large' ]:
            consquent_with_term, term = int2consq(fuzzy_controller_param[pointer], fuzzy_consequent)
            rule_list.append(ctrl.Rule(
                antecedent_1[mf_3],  consquent_with_term
            ))
            print(f'{str(antecedent_1[mf_3])} --> {str(consquent_with_term)}')
            pointer += 1
    else:
        raise RuntimeError
    
    assert len(fuzzy_controller_param) == pointer, "the length of fuzzy_controller_param is not correct"
