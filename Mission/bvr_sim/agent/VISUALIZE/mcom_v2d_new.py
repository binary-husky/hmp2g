...
def disable_scale(plotwin):
    # plotwin.enableAutoRange('xy', False, _callSync='off')
    return 
def plotwin_config(plotwin):
    # plotwin.setAspectLocked(_callSync='off')
    # plotwin.setYRange(-150000, +150000, padding=0, _callSync='off')
    # plotwin.setXRange(-150000, +150000, padding=0, _callSync='off')
    # plotwin.setConfigOptions(antialias=True)
    return

def add_breaking_line(line_handle, data_N2):
    is_nan = np.isnan(data_N2).any(-1)
    is_nan_2 = is_nan.copy()
    is_nan_2 = np.concatenate((is_nan_2[1:],[False])) | is_nan_2
    connect = is_nan_2[~is_nan]
    connect = (~connect).astype(np.int32)
    data_N2_ = data_N2[~is_nan]
    line_handle.setData(data_N2_, connect=connect, _callSync='off')
    return

def adjust_line(plotwin, line_handle):
...
class v2d_family():
    ...
    def get_new_line_handle(self,char_color):
        # self.plotwin.setYRange(-150000, +150000)
        # self.plotwin.setXRange(-150000, +150000)
        return self.plotwin.plot(pen=char_color, skipFiniteCheck=True)

    def clear_all(self):
        self.plotwin.clear(_callSync='off')
    ...
    def __init__(self, draw_mode) -> None:
        ...
        proc = mp.QtProcess()
        self.rpg = proc._import('pyqtgraph')
        self.plotwin = self.rpg.plot()
        self.plotwin.resize(600,600)
        self.plotwin.setWindowTitle('pyqtgraph example: Plotting')
        self.plotwin.setAspectLocked(_callSync='off')
        self.plotwin.setYRange(-150000, +150000, padding=0, _callSync='off')
        self.plotwin.enableAutoRange('xy', False, _callSync='off')
        assert draw_mode=='pyqtgraph', ('only support pyqtgraph')
        pass
    ...
    def init_fig(self):
        return
        self.v2d_fig_handle = plt_figure(V2dPlotFig, figsize=(8, 8), dpi=100)
    ...
    def add_to_style(self, char_index, char_color):
        if char_color not in self.style_list.keys():
            ...
            self.style_list[char_color]['plot_handle'] = self.get_new_line_handle(char_color) # self.plotwin.plot()
        # 样式已经被注册
    ...
    def v2d_draw(self):
        ...
        for style in self.style_list.keys(): 
            ...
            data_N2 = np.stack((x_data_concat,y_data_concat)).T
            add_breaking_line(line_handle, data_N2)
            
            adjust_line(self.plotwin, line_handle)
        ...
        disable_scale(self.plotwin)
        plotwin_config(self.plotwin)
    ...
    def v2d_line_object(self, indexA, indexB):
        ...
        if line_name not in self.trival_line_list:
            ...
            self.trival_line_list[line_name] = self.get_new_line_handle() # self.v2d_fig_handle.gca().plot([x1,x2],[y1,y2], lw=1, c='k')[0]
        else:
            add_breaking_line(
                line_handle = self.trival_line_list[line_name],
                data_N2 = np.array([[x1,x2],[y1,y2]]).T
            )
