...
"""
# def get_ndarray_indicator(shape, dtype, shm_start, shm_end):
#     return ()
...
def convert_ndarray(numpy_ndarray, shm_pointer, shm):
    ...
    # NID = ndarray_indicator(shape, dtype, shm_start, shm_pointer+nbyte, count)
    NID = ('NID', shape, dtype, shm_pointer, nbyte//dtype.itemsize)
        ...
        if isinstance(obj, list):
            for i_th in range(len(obj)):
                if isinstance(obj[i_th], list):
                    ...
                    if isinstance(obj[i_th][0], str) and obj[i_th][0] == 'NID':
                        NID = obj[i_th]     # NID = (shape, dtype, shm_pointer, shm_pointer+nbyte, count)
                        # NID = (shape 1, dtype 2, shm_pointer 3, shm_pointer+nbyte 4, count 5)
                        obj[i_th] = np.frombuffer(shm, dtype=NID[2], offset=NID[3], count=NID[4]).reshape(NID[1])
                    else:
                        item2 = list(obj[i_th])
