...
class 三体运动仿真():
    def __init__(self) -> None:
        ############### 初始条件 initial position ###########
        self.位置 = 2*np.array([    
            [cos(0),    sin(0), 0],           [cos(2*PI/3),    sin(2*PI/3),  0],             [cos(-2*PI/3),      sin(-2*PI/3), 0],  ])  # the position of entities
        self.速度 = 1.074*np.array([ 
            [cos(0+PI/2), sin(+PI/2), 0],     [cos(2*PI/3+PI/2),    sin(2*PI/3+PI/2),  0],   [cos(-2*PI/3+PI/2),    sin(-2*PI/3+PI/2), 0],  ])  # the position of entities
        self.质量 = np.array(
            [     1,               1,              1      ])  # the position of entities
        self.n体 = len(self.位置); assert len(self.位置) == len(self.质量)
        self.step = 0; self.仿真间隔 = 0.001
    ...
    def physic_sim(self):
        ...
        #GMm/(r*r)
        G = 4
        gravity = (G*mass2mass)/(dis.squeeze(-1)**2 + 1e-10)
    ...
    def render(self):
        ...
        for index in range(self.n体):
            self.可视化桥.发送几何体(
                ...
                label_color='Black',  opacity=0.76)
        self.可视化桥.结束关键帧()
