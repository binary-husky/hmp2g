...
def make_collective_assault_env(env_id, rank):
    ...
    from .envs.collective_assault_env import collective_assaultEnvV1
    scenario = collective_assaultEnvV1()
    # create world
    world = scenario.world
    world.max_time_steps = ScenarioConfig.MaxEpisodeStep
    # create multiagent environment
    if ScenarioConfig.benchmark:
        env = collective_assaultGlobalEnv(world, scenario, scenario.benchmark_data, id=rank)
    else:
        env = collective_assaultGlobalEnv(world, scenario, id=rank)
    return env
