...
class ScenarioConfig(object):  
    ...
    N_AGENT_EACH_TEAM = [team_blue, team_red]
    N_AGENT_EACH_TEAM_cv = ChainVar(lambda team_blue, team_red:[team_blue, team_red,], chained_with=['team_blue', 'team_red']) 

    AGENT_ID_EACH_TEAM = [range(0,team_blue),     range(team_blue,team_blue+team_red)]
    AGENT_ID_EACH_TEAM_cv = ChainVar(lambda team_blue, team_red:[range(0,team_blue), range(team_blue,team_blue+team_red)], chained_with=['team_blue', 'team_red']) 

    TEAM_NAMES = [ 'ALGORITHM.Starcraft.star_foundation->StarFoundation', ]
    obs_vec_length = 16

    # num_entity = team_red
    # num_entity_cv = ChainVar(lambda team_red:team_red, chained_with=['team_red']) 

    # num_object = team_red + team_blue
    # num_object_cv = ChainVar(lambda team_red, team_blue: team_red+team_blue, chained_with=['team_red','team_blue']) 

    # uid_dictionary = {'agent_uid':range(0,50), 'entity_uid':range(50,100), 'guards_uid':range(0,50), 'attackers_uid':range(50,100)}
    # uid_dictionary_cv = ChainVar(lambda team_blue,team_red :{
    #     'agent_uid':range(0,team_blue), 
    #     'entity_uid':range(team_blue,team_red+team_blue), 
    #     'guards_uid':range(0,team_blue), 
    #     'attackers_uid':range(team_blue,team_red+team_blue)
    #     }, chained_with=['team_blue','team_red']) 
...
def make_collective_assult_env(env_id, rank):
    ...
    scenario = collective_assultEnvV1( 
                                team_blue_n_agent=ScenarioConfig.team_blue, 
                                team_red_n_agent = ScenarioConfig.team_red, 
...
class collective_assultGlobalEnv(gym.Env):
    ...
    def render(self):
        ...
        for index, agent in enumerate(self.world.agents):
            ...
            color = 'red' if agent.belong_red_team else 'blue'
            base_color = 'LightPink' if agent.belong_red_team else 'CornflowerBlue'
            if not agent.alive:
                color = "#330000" if agent.belong_red_team else "#000033"
                base_color = "#330000" if agent.belong_red_team else "#000033"
            # base_color = base_color if agent.alive else 'black'

            shape3d = 'cone' # if agent.belong_blue_team else 'cone'
            ...
            if agent.wasHitBy is not None:
                flash_type = 'lightning' if agent.belong_red_team else 'lightning'
                flash_color = 'DeepSkyBlue' if agent.belong_red_team else 'Magenta'
        ...
        if self.s_cfg.Terrain_DEBUG:
            ...
            # blue_pos = np.array([agent.pos for agent in self.world.agents if agent.belong_blue_team])
            # red_pos = np.array([agent.pos for agent in self.world.agents if agent.belong_red_team])
            # distance = distance_matrix_AB(blue_pos, red_pos)
            for blue_agent in [agent for agent in self.world.agents if agent.belong_blue_team]:
                for red_agent in [agent for agent in self.world.agents if agent.belong_red_team]:
