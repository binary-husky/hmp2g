...
import numpy as np
import sys
sys.path.append('./MISSION/collective_assault')
...
<<<<<<< HEAD:MISSION/dca/collective_assult_parallel_run.py
...
=======
def make_collective_assault_env(env_id, rank):
    # scenario = gym.make('collective_assault-v1')
    from .collective_assault_env import collective_assaultEnvV1
    scenario = collective_assaultEnvV1( numguards=ScenarioConfig.num_guards, 
>>>>>>> master:MISSION/dca/collective_assault_parallel_run.py
    ...
    if ScenarioConfig.benchmark:
        env = collective_assaultGlobalEnv(world, scenario.reset_world, scenario.reward, scenario.observation, scenario.benchmark_data, id=rank)
    else:
        env = collective_assaultGlobalEnv(world, scenario.reset_world, scenario.reward, scenario.observation, id=rank)
    return env


# environment for all agents in the multiagent world
# currently code assumes that no agents will be created/destroyed at runtime!
class collective_assaultGlobalEnv(gym.Env):
    metadata = {
    ...
    def step(self, action_n):
        ...
        self.world.step() # world is the collective_assault-v0 environment, step function is in core.py file
