...
from .UhmapFlagCaptureConf import SubTaskConfig
...
class UhmapFlagCapture(UhmapCommonFn, UhmapEnv):
    ...
    def gen_reward_and_win(self, resp):
        ...
        WinningResult = None
        flag = [k for k in resp['dataGlobal']['keyObjArr'] if k['className']=='FlagToCapture_C']
        assert len(flag) == 1
        flag = flag[0]
        info = flag['rSVD1']

        if info == 'Captured By No Team': 
            pass
        if info == 'Captured By Team 0': 
            reward[0] += 0.005
            self.dominate_team_score[0] += 1
        if info == 'Captured By Team 1': 
            reward[1] += 0.005
            self.dominate_team_score[1] += 1
        if info == 'Captured By Team 2': 
            reward[2] += 0.005
            self.dominate_team_score[2] += 1
