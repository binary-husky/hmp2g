base = """
{
    "config.py->GlobalConfig": {
        ...
        "backup_files": [
            "ALGORITHM/coop_space_forwarding",
    ...
    "MISSION.sr_tasks.multiagent.scenarios.cargo.py->ScenarioConfig": {
        ...
        "render": "False",
        "TEAM_NAMES": [
            "ALGORITHM.coop_space_forwarding.reinforce_foundation->ReinforceAlgorithmFoundation"
        ]
    },
    "ALGORITHM.coop_space_forwarding.reinforce_foundation.py->CoopAlgConfig": {
...
sum_note = "cargo-coop_space_forwarding_origin_+newnorm"
conf_override = {
...
def check_file_mod_time(base_conf):
    ...
    print(f'experiment note is {sum_note}')
    input('Confirm ?')
...
if __name__ == '__main__':
    ...
    if not args.debug:
        ...
        shutil.copyfile(file, os.path.join(os.path.dirname(file), f'private {time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())} batch_experiment_backup {sum_note}.py'))
