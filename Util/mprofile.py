...
if not os.path.exists('temp/%s'%exp_log_dir):
    os.makedirs('temp/%s'%exp_log_dir)
exp_json_dir = log_dir+'exp_json'
if not os.path.exists('temp/%s'%exp_json_dir):
    os.makedirs('temp/%s'%exp_json_dir)


new_json_paths = []
for i in range(n_run):
    conf = copy.deepcopy(base_conf)
    new_json_path = 'temp/%s/run-%d.json'%(exp_json_dir, i+1)
...
def worker(ith_run):
    log_path = open('temp/%s/run-%d.log'%(exp_log_dir, ith_run+1), 'w+')
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if any(is_alive):
            ...
            print靛('current scipt:%s, current log:%s'%(os.path.abspath(__file__), 'temp/%s/run-%d.log'%(exp_log_dir, ith_run+1)))
            time.sleep(60)
        else:
            break
